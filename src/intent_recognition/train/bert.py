"""训练脚本：微调本地 BERT（bert-base-chinese）做意图识别。
...
from ..data.loader import to_class_label_dataset
...
class EpochSummaryCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs) -> None:
        ...
        if "eval_accuracy" in metrics:
            parts.append(f"acc {float(metrics['eval_accuracy']):.4f}")
...
def train() -> None:
    ...
    # 1. 加载数据并 8:2 划分（按标签分层，保证小类在测试集里也有代表性）
    ds = to_class_label_dataset(model_cfg, data_cfg)
    split = ds.train_test_split(
        test_size=data_cfg.test_ratio, seed=42, stratify_by_column="label"
    )
